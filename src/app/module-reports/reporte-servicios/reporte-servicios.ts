...
import { ReportServices, ServicioItem, ResumenServicios } from '../services/report.service';
...
export class ReporteServicios implements OnInit {
  ...
  dataSource = new MatTableDataSource<ServicioItem>([]);
  ...
  lineChart: ApexChart = { type: 'line', height: 280, id: 'servicesMonthChart', toolbar: { show: true } };
  lineXAxis: ApexXAxis = { categories: [] };

  topSeries: ApexAxisChartSeries = [{ name: 'Top por valor', data: [] }];
  topChart: ApexChart = { type: 'bar', height: 280, id: 'servicesTopChart', toolbar: { show: true } };
  topXAxis: ApexXAxis = { categories: [] };
  topPlotOptions: ApexPlotOptions = { bar: { horizontal: true } };
  hasTopData = false;
  hasTopNonZero = false;
  aggregated: ServicioItem[] = [];
  ...
  async downloadCsv(): Promise<void> {
    ...
    try {
      ...
      this.reports.downloadServiciosCsv(params).subscribe(async (blob) => {
        const fileUrl = URL.createObjectURL(blob);
        const fileName = `servicios_${new Date().toISOString().slice(0, 10)}.csv`;
  ...
  async exportPDF(): Promise<void> {
    ...
    try {
      const doc = new jsPDF();
      doc.text('Reporte de Servicios', 14, 10);
      ...
      autoTable(doc, {
        ...
        body: this.dataSource.data.map((s) => [
          ...
          s.nombre,
          s.unidad,
          s.cantidad_vendida?.toString() || '0',
          s.subtotal?.toString() || '0',
          s.impuestos?.toString() || '0',
          s.total?.toString() || '0',
          s.precio_promedio?.toString() || '0',
        ]),
      });

      const pdfBlob = doc.output('blob');
      const fileUrl = URL.createObjectURL(pdfBlob);
      const fileName = `Reporte_Servicios_${Date.now()}.pdf`;
  ...
  async exportChartsToPDF(): Promise<void> {
    ...
    try {
      ...
      y = await addImg('servicesTopChart', 'Top 5 Servicios por Unidades Vendidas');
      y = await addImg('servicesMonthChart', 'Ventas de Servicios por Mes');

      const pdfBlob = doc.output('blob');
      const fileUrl = URL.createObjectURL(pdfBlob);
      const fileName = `graficos_servicios_${new Date().toISOString().slice(0, 10)}.pdf`;
  ...
  async exportFullReportPDF(): Promise<void> {
    ...
    try {
      const doc = new jsPDF('p', 'mm', 'a4');
      let y = 10;
      const title = 'Reporte Completo de Servicios';
      doc.setFontSize(16);
      doc.text(title, 10, y);
      y += 8;
      ...
      y = await addImg('servicesMonthChart', 'Ventas por Mes');
      y = await addImg('servicesTopChart', 'Top Servicios por Unidades Vendidas');

      // Agregar tabla de top servicios
      doc.setFontSize(12);
      doc.text('Resumen de Top Servicios', 10, y);
      y += 5;

      const topServices = [...this.dataSource.data]
        .sort((a, b) => (b.cantidad_vendida || 0) - (a.cantidad_vendida || 0))
        .slice(0, 10);

      const topHeader = [['Servicio', 'Unidades Vendidas', 'Total Vendido']];
      const topBody = topServices.map(s => [
        s.nombre,
        String(s.cantidad_vendida || 0),
      ...
      // Agregar tabla completa de servicios
      doc.setFontSize(12);
      doc.text('Lista Completa de Servicios', 10, afterTop + 6);
      ...
      const pdfBlob = doc.output('blob');
      const fileUrl = URL.createObjectURL(pdfBlob);
      const fileName = `reporte_completo_servicios_${new Date().toISOString().slice(0, 10)}.pdf`;
  ...
  private load(): void {
    ...
    this.invoiceService.getInvoices(filters).pipe(catchError(() => of([]))).subscribe((invoices: Invoice[]) => {
      if (!invoices || invoices.length === 0) {
        ...
        this.reports.getServicios(params).pipe(catchError(() => of({ data: [] as ServicioItem[], total: 0 }))).subscribe(({ data }) => {
          const mapped: ServicioItem[] = (data || []).map((item: any) => ({
            codigo: item.codigo || item.code || item.service_code || '',
            nombre: item.nombre || item.name || item.service_name || '',
      ...
      forkJoin(detailRequests).subscribe((fulls) => {
        const grouped = new Map<string, ServicioItem>();
        for (const inv of fulls.filter(Boolean) as Invoice[]) {
          ...
          for (const d of details) {
            ...
            if (!code && !item?.service_code) continue;
  ...
  private loadResumen(): void {
    ...
    this.reports.getResumenServicios(params).subscribe((r: ResumenServicios) => {
      ...
      this.topXAxis = { categories: tops.map((t: any) => t.servicio) };
