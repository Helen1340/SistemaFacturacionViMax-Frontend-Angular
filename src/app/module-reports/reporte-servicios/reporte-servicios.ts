...
export class ReporteServicios implements OnInit {
  ...
  downloadCsv(): void {
    ...
    this.reports.downloadServiciosCsv(params).subscribe((blob) => {
      saveAs(blob, `servicios_${new Date().toISOString().slice(0,10)}.csv`);
    });
  }
  ...
  exportChartsToPDF(): void {
    ...
    ApexCharts.exec('servicesTopChart', 'dataURI').then((t: any) => {
      const img2 = t?.imgURI;
      if (img2) {
        doc.text('Top 5 servicios', 10, 10);
        doc.addImage(img2, 'PNG', 10, 15, 190, 80);
      }
      doc.save(`graficos_servicios_${new Date().toISOString().slice(0,10)}.pdf`);
    });
  }
}